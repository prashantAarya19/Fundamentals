package dp;

import java.util.Arrays;

public class CutRodSegment {
    public static int cutSegments(int n, int x, int y, int z) {
        int result = solveRecursive(n, x, y, z);
        if(result < 0) {
            return 0;
        }
        return result;
    }

    public static int solveRecursive(int n, int x, int y, int z) {
        if(n == 0)
            return 0;
        if(n < 0)
            return Integer.MIN_VALUE;

        int case1 = solveRecursive(n - x, x, y, z) + 1;
        int case2 = solveRecursive(n - y, x, y, z) + 1;
        int case3 = solveRecursive(n - z, x, y, z) + 1;

        return Integer.max(case1, Integer.max(case1, case2));
    }

    public static int solveDP(int n, int x, int y, int z, int[] dp) {
        if(n == 0)
            return 0;
        if(n < 0)
            return Integer.MIN_VALUE;
        if(dp[n] != -1)
            return dp[n];

        int case1 = solveDP(n - x, x, y, z, dp) + 1;
        int case2 = solveDP(n - y, x, y, z, dp) + 1;
        int case3 = solveDP(n - z, x, y, z, dp) + 1;

        dp[n] = Integer.max(case1, Integer.max(case2, case3));
        return dp[n];
    }

    public static int solveTab(int n, int x, int y, int z) {
        int[] dp = new int[n + 1];
        Arrays.fill(dp, Integer.MIN_VALUE);
        dp[0] = 0;

        for(int i = 1; i <= n; i++) {
            if(i - x >= 0)
                dp[i] = Integer.max(dp[i], dp[i - x] + 1);
            if(i - y >= 0)
                dp[i] = Integer.max(dp[i], dp[i - y] + 1);
            if(i - z >= 0)
                dp[i] = Integer.max(dp[i], dp[i - z] + 1);
        }

        int result = dp[n];
        if(result < 0)
            return 0;
        else
            return result;
    }
}
